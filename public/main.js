...
ipcMain.handle("get/file", async (event) => {
  let results = await dialog.showOpenDialog({
    properties: ["openFile"],
    filters: [{ name: "Custom File Type", extensions: ["doc", "docx", "pdf"] }],
  });
...
ipcMain.handle("login-validation", async (event, values) => {
  try {
    let responseData = await axios.post(`${BASE_API_}/admin/v1/login`, values);
...
ipcMain.handle("get-download-file", async (event, values) => {
  try {
    let responseData = await axios.get(`${BASE_API_}/docs/download/${values}`);
  ...
  if (dialogSaves.canceled) {
    ...
    fs.writeFile(
      ...
      function (err) {
        ...
        //return;
        const newoptions = { message: `File Saved - ${dialogSaves.filePath}` };
