...
const FacilitatorTable = ({
    ...
    useEffect(() => {
        let allQueriedItems = allfacilitatorItems.items.filter((datas) => {
            return datas
        })
        /** initial items  */
        //items collected
    ...
    const handleFilters = () => {
        const searchResults = allDisplayData.allItems.filter((data1, index) => {
            /** student name */
            if (filterInfo[0].title === 'Facilitator Name') {
                let name =
                    `${data1.firstname.toLowerCase()}` +
                    ' ' +
                    `${data1.lastname.toLowerCase()}`
                let tname = `${
                    data1.jobtitle.toLowerCase() + ' ' + name.toLowerCase()
                }`
                                        ...
                                        <>
                                            {searchData.items.map(
                                                (data, index) => {
                                                    ...
                                                    let getLogins =
                                                        allLoginActivityItems.items.filter(
                                                            (datas) => {
                                                                if (
                                                                    datas
                                                                        .adminId
